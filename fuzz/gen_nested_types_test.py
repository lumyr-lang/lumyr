...
def main():
    ...
    for t, lit, cmpv, _, avtype, _ in TYPES:
        ...
        o.append('check(type(' + a + ') == "array", "B.' + tg + ': container");')
    ...
    o.append("")
    o.append("// ===== E2. 泛型数组/map 字段声明 =====")
    o.append("struct GTyped {")
    o.append("    nums: <int>array;")
    o.append("    mp: <string, int>map;")
    o.append("}")
    o.append("gt = <GTyped>{nums: <int>[1, 2, 3], mp: <string, int>{\"a\": 1, \"b\": 2}};")
    o.append('check(type(gt.nums) == "array", "E2.typed array field");')
    o.append('check(gt.nums[0] == 1, "E2.typed array elem0");')
    o.append('check(gt.nums[2] == 3, "E2.typed array elem2");')
    o.append("gt.nums[1] = 20;")
    o.append('check(gt.nums[1] == 20, "E2.typed array modify");')
    o.append('check(type(gt.mp) == "map", "E2.typed map field");')
    o.append('check(gt.mp["a"] == 1, "E2.typed map value a");')
    o.append('check(gt.mp["b"] == 2, "E2.typed map value b");')
    o.append("gt.mp[\"c\"] = 3;")
    o.append('check(gt.mp["c"] == 3, "E2.typed map add");')
    o.append("")
    o.append("class GTypedC {")
    o.append("    nums: <int>array;")
    o.append("    mp: <string, int>map;")
    o.append("    func GTypedC() {")
    o.append("        self.nums = <int>[10, 20, 30];")
    o.append("        self.mp = <string, int>{\"x\": 100};")
    o.append("    }")
    o.append("    func sum() {")
    o.append("        return self.nums[0] + self.nums[1] + self.nums[2];")
    o.append("    }")
    o.append("}")
    o.append("gtc = GTypedC();")
    o.append('check(type(gtc.nums) == "array", "E2.class typed array field");')
    o.append('check(gtc.sum() == 60, "E2.class method");')
    o.append('check(type(gtc.mp) == "map", "E2.class typed map field");')
    o.append('check(gtc.mp["x"] == 100, "E2.class typed map value");')
    o.append("gtc.nums[0] = 0;")
    o.append('check(gtc.sum() == 50, "E2.class typed array modify");')
    o.append("gtc.mp[\"y\"] = 200;")
    o.append('check(gtc.mp["y"] == 200, "E2.class typed map add");')
    o.append("")
    o.append("type GTypedT {")
    o.append("    nums: <int>array,")
    o.append("    mp: <string, int>map")
    o.append("}")
    o.append("gtt = <GTypedT>{nums: <int>[7, 8], mp: <string, int>{\"k\": 9}};")
    o.append('check(type(gtt.nums) == "array", "E2.type typed array field");')
    o.append('check(gtt.nums[0] == 7, "E2.type typed array elem");')
    o.append("gtt.nums[1] = 80;")
    o.append('check(gtt.nums[1] == 80, "E2.type typed array modify");')
    o.append('check(type(gtt.mp) == "map", "E2.type typed map field");')
    o.append('check(gtt.mp["k"] == 9, "E2.type typed map value");')
    o.append("gtt.mp[\"j\"] = 10;")
    o.append('check(gtt.mp["j"] == 10, "E2.type typed map add");')
    o.append("")
